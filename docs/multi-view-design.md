...
字段池面板内容：
┌──────────────────────────────────┐
│  字段池              6 个字段      │  ← 只统计叶子字段（实际表单输入组件）
├──────────────────────────────────┤
│  ┌ 申请人           f1           │  ← EpicTree 树形展示
│  ├ 请假事由         f2           │     与 epActionModal 风格一致
│  ├ 请假天数         f3           │
│  └ 请假类型         f4           │
│                                  │
│  数据来源：始终是数据模型          │
...
- **字段池**（新增）：列出数据模型中的全部字段，使用 `EpicTree` 组件渲染
  ...
  - 统计只计算叶子节点（实际的表单输入组件），排除容器节点（row/col 等）
  - 模型模式下点击字段可在右侧面板编辑属性
  - 视图模式下可从字段池拖拽字段到画布（加入当前视图）
...
| 约束 | 说明 |
|------|------|
| **字段默认不出现** | 在数据模型中新增字段后，不自动出现在任何视图中，需从字段池拖入或通过 API 加入 |
